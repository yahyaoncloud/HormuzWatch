...
deployed to the service — only the saved .joblib files are consumed by app.py.
...
from lib.scoring import _normalize_raw_score, fitted_score_bounds  # noqa: E402

from lib.logger import get_logger

logger = get_logger("hormuzwatch.train")
...
def _load_data(path: Path, feature_cols: list[str]) -> np.ndarray:
    ...
    service deployment.  Install it with: ``pip install pandas`` locally.
    """
    try:
        ...
        raise ImportError(
            ...
            "(It is NOT bundled into the deployment.)"
        )
...
def train(
    ...
    if_bounds = fitted_score_bounds(raw_if_scores)
    lof_bounds = fitted_score_bounds(raw_lof_scores)
    norm_if = np.vectorize(_normalize_raw_score)(raw_if_scores, *if_bounds)
    norm_lof = np.vectorize(_normalize_raw_score)(raw_lof_scores, *lof_bounds)
    ...
    bundle = {
        ...
        "score_bounds": {"iforest": if_bounds, "lof": lof_bounds},
        ...
        "training_metadata": {
            ...
            "calibration_status": "in_sample_labelled" if labels is not None else "uncalibrated_rank",
        },
    }
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Produces ensemble .joblib artifacts for inference."
        ),
