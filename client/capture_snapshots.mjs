...
const OUT_DIRS = [
  path.resolve('./public/assets/snapshots'),
  path.resolve('../assets/snapshots'),
];
...
async function captureSnapshots() {
  console.log(`[Snapshots] Launching Playwright Chromium to capture snapshots from ${BASE_URL}...`);
  ...
  for (const route of routes) {
    ...
    try {
      ...
      for (const dir of OUT_DIRS) {
        ...
        console.log(`[✓] Saved snapshot: ${dest}`);
      }
    } catch (err) {
