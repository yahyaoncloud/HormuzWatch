  ...
  return (
    <PageContainer>
      ...
      >
        <Prose>
          <p>
            ...
            corridor, the Suez Canal approaches, and the Persian Gulf collectively carry over 20% of
            global energy trade and container traffic.
          </p>
          <p>
            Operating within these contested maritime corridors presents acute challenges:
            electronic warfare, GPS spoofing, automated identification system (AIS) transponder
            blackouts, and asymmetric tactical threats. HormuzWatch provides a calm,
            methodologically transparent, and continuously updated record of what is happening at
            sea.
          </p>
          <blockquote>
            Our objective is not to replace operational naval command centers. It is to serve as an
            authoritative public reference: a calm, well-documented account that analysts, logistics
            coordinators, academic researchers, and journalists can consult with confidence.
          </blockquote>
        </Prose>
      ...
      <Section
        ...
        subtitle="Why contested littoral waterways are difficult to observe"
      >
        <Prose>
          <p>
          ...
          <ol>
            <li>
              <strong>Telemetry Fragmentation</strong> — Vessel positions (AIS), aircraft tracks
              (ADS-B), geopolitical events (GDELT), satellite fire detection (NASA FIRMS), and
              maritime news exist across incompatible schemas with no unified fusion.
            </li>
            <li>
              <strong>Institutional Barriers</strong> — Commercial tracking platforms gate vital
              intelligence behind opaque subscriptions or classified military command feeds.
            </li>
            <li>
              <strong>Alert Fatigue and Raw ML Failure</strong> — Commercial tankers routinely
              maneuver for safety or weather. Naive threshold rules generate thousands of false
              alarms, while uncalibrated raw anomaly scores produce severe overconfidence.
            </li>
          </ol>

          <div className="prose-callout info">
            <strong>Scope Boundaries:</strong> HormuzWatch prioritizes scientific reproducibility
            and calibration accuracy. We report calibrated probabilities alongside TreeSHAP feature
            attributions so every alert can be audited.
          </div>
        </Prose>
      </Section>
      ...
      <Section
        id="approach"
        title="Methodology"
        subtitle="Open telemetry, calibrated ML ensemble, tri-partite threat fusion"
      >
        <Prose>
          ...
          <p>
            Six integration workers pull from public telemetry feeds continuously. Ingested pings
            are managed in an in-memory Time-Series State Manager (TSM) maintaining online moments
            via Welford's algorithm. Circular angular statistics on the 1-sphere manifold ($S^1$)
            eliminate $359^\circ \leftrightarrow 0^\circ$ boundary discontinuities.
          </p>

          <h3>2. Dual Ensemble & Isotonic Calibration (Python 3.11 / gRPC :8091)</h3>
          <p>
            A synergistic ensemble combining <strong>Isolation Forest (200 trees)</strong> for
            global geometric space partitioning with <strong>Local Outlier Factor (k=20)</strong>{' '}
            for local neighborhood density estimation. Raw scores are blended (0.55 * IF + 0.45 *
            LOF) and transformed via non-parametric <strong>Isotonic Regression</strong>, reducing
            Expected Calibration Error (ECE) by <strong>69.2%</strong> (down to 4.57%).
          </p>

          <h3>3. Tri-Partite Threat Intelligence Fusion</h3>
          <p>
            To prevent single points of failure, the Go backend synthesizes risk across three
          ...
          <p>
            This guarantees that statistical outliers are never confused with hostile rogue intent
            without geopolitical and regulatory corroboration.
          </p>
        </Prose>
      ...
      >
        <Prose>
          <p>
            HormuzWatch is an independently developed open-source geospatial intelligence
            initiative. The timeline below documents major architectural milestones and feature
            deliveries.
          </p>
        </Prose>
      ...
      <Section id="data" title="Data Pipeline" subtitle="Six public sources, one unified record">
        <Prose>
          <p>
            Every observation in HormuzWatch begins as a public feed. We deliberately prefer open
          ...
          <p>
            Telemetry records are cached in memory for sub-10ms queries. Historical observation
            windows feed the Continuous Training (CT) pipeline with automated Population Stability
            Index (PSI) and Kolmogorov-Smirnov drift monitoring.
          </p>

          <div className="prose-callout warn">
            <strong>Operational Disclaimer:</strong> HormuzWatch provides geospatial intelligence
            for situational awareness and research. Always consult official International Maritime
            Organization (IMO) and national coastal authorities for navigation commands.
          </div>
        </Prose>
      </Section>
