import { AlertTriangle, Anchor, Gauge, Layers, Plane, Radio, Ship, TrendingUp } from 'lucide-react';
...
const ANALYTICS_TODOS: TodoItem[] = [
  {
    id: 'y1',
    title: 'Regional Threat Density & Category Charts',
    category: 'UI & UX',
    completed: true,
    notes: 'Visual progress bar density & category article distribution',
  },
  {
    id: 'y2',
    title: 'Automated PDF Executive Briefing Generator',
    category: 'API & Data',
    completed: false,
    notes: 'Connect jsPDF / html2canvas to generate downloadable PDF summaries',
  },
  {
    id: 'y3',
    title: 'OSINT Source Reliability Scatter Matrix',
    category: 'ML & Anomaly',
    completed: false,
    notes: 'Scatter plot comparing scrape volume against HTTP error rate per feed',
  },
  {
    id: 'y4',
    title: 'Choropleth Threat Heatmap Integration',
    category: 'UI & UX',
    completed: false,
    notes: 'Shaded country map visualization reflecting real-time threat scores',
  },
];
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-[var(--color-fg)]">
            Analytics & Strategic Reports
          </h1>
          <p className="font-ui text-sm text-[var(--color-fg-muted)] mt-1">
            Live pipeline telemetry — computed in-memory from active track state. Pushed via
            WebSocket every 1s.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-4 space-y-1">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-[11px] font-mono text-[var(--color-fg-muted)]">
                {formatPct(anomalyPct)} of all tracks
              </p>
            </>
          )}
        </div>
        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="space-y-3 pt-2">
              {[
                {
                  label: 'Anchored (<0.5 kn)',
                  count: stats?.anchoredCount ?? 0,
                  pct: anchoredPct,
                  color: 'bg-slate-500',
                  icon: Anchor,
                },
                {
                  label: 'Slow (0.5–3 kn)',
                  count: stats?.slowCount ?? 0,
                  pct: slowPct,
                  color: 'bg-amber-500',
                  icon: Gauge,
                },
                {
                  label: 'Maneuvering (3–8 kn)',
                  count: stats?.maneuveringCount ?? 0,
                  pct: maneuveringPct,
                  color: 'bg-blue-500',
                  icon: Ship,
                },
                {
                  label: 'Transiting (>8 kn)',
                  count: stats?.transitingCount ?? 0,
                  pct: transitingPct,
            ...
            <div className="space-y-3 pt-2">
              {[
                ...
                },
                {
                  ...
                  pct:
                    totalTracks > 0
                      ? formatPct(((stats?.highAnomalyCount ?? 0) / totalTracks) * 100)
                      : '0.0%',
                  icon: AlertTriangle,
                  color: 'text-red-400',
      ...
      {stats?.updatedAt && (
        <div className="text-center text-[10px] font-mono text-[var(--color-fg-muted)]">
          Last push: {new Date(stats.updatedAt).toLocaleTimeString()} · WebSocket → Zustand · Every
          1s
        </div>
      )}
