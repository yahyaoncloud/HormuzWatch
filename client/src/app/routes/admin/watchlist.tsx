...
const WATCHLIST_TODOS: TodoItem[] = [
  {
    id: 'w1',
    title: 'Surveillance Watchlist Roster & Add Wizard',
    category: 'UI & UX',
    completed: true,
    notes: 'Roster view with modal wizard for adding IMO/ICAO entities',
  },
  {
    id: 'w2',
    title: 'Backend GET & POST /watchlist Endpoints',
    category: 'API & Data',
    completed: false,
    notes: 'Connect Go REST API endpoints for persistent watchlist database storage',
  },
  {
    id: 'w3',
    title: 'Automated Anomaly Score Watchlist Rule',
    category: 'ML & Anomaly',
    completed: false,
    notes: 'Auto-add targets to watchlist when anomaly score exceeds set threshold',
  },
  {
    id: 'w4',
    title: 'Zone Entry Webhook & Push Alerts',
    category: 'Security & Auth',
    completed: false,
    notes: 'Trigger real-time alert when watchlisted asset enters monitored sector',
  },
];
...
export default function AdminWatchlist() {
  ...
  const [newItem, setNewItem] = useState({
    id: '',
    name: '',
    type: 'Vessel' as 'Vessel' | 'Aircraft',
    reason: '',
    riskLevel: 'High' as 'Critical' | 'High' | 'Medium',
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-ui text-sm text-[var(--color-fg-muted)] mt-1">
            Managed directory of flagged maritime vessels and aviation targets subject to continuous
          ...
          <tbody>
            {watchlist.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-xs text-[var(--color-fg-muted)] max-w-sm">
                  {item.reason}
                </td>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewItem((prev) => ({
                        ...prev,
                        type: e.target.value as 'Vessel' | 'Aircraft',
                      }))
                    }
                    className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-xs font-ui text-[var(--color-fg)] focus:outline-none focus:border-[var(--color-primary-600)]"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewItem((prev) => ({ ...prev, riskLevel: e.target.value as any }))
                    }
                    className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-xs font-ui text-[var(--color-fg)] focus:outline-none focus:border-[var(--color-primary-600)]"
                  >
