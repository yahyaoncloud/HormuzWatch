import { Check, Copy, Key, Laptop, RefreshCw, ShieldCheck, User } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="font-ui text-sm text-[var(--color-fg-muted)] mt-1">
          Manage root administrator credentials, active session security, and API authentication
          tokens.
        </p>
      </div>
      ...
      <div className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-[var(--color-fg-muted)] mt-0.5">
              {env.auth.adminDisplayEmail}
            </p>
            <p className="font-ui text-xs text-[var(--color-fg-muted)] mt-1">
              Role: SuperAdmin &bull; Full Access
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 space-y-4">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-3">
            <div>
              <label className="block text-xs font-ui text-[var(--color-fg-muted)] mb-1">
                Current Password
              </label>
              <input
                type="password"
        ...
        <div className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6 space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <label className="block text-[11px] font-mono text-[var(--color-fg-muted)]">
                Active Secret Key
              </label>
              <div className="flex items-center gap-2">
                <input
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold font-ui text-[var(--color-fg)]">
                  Current Active Session (This Device)
                </p>
                <p className="text-[11px] font-mono text-[var(--color-fg-muted)]">
                  IP: 127.0.0.1 &bull; Chrome on Windows
                </p>
              </div>
            </div>
