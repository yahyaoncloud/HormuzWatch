...
import { type SectorData, SectorStatusPanel } from './SectorStatusPanel';
...
export function IntelligenceDashboard({ onViewOnMap, className }: IntelligenceDashboardProps) {
  ...
  // Derive counts
  const activeAnomaliesCount = anomaliesList.filter(
    (a) => a.score >= 1 || (a.severity && a.severity !== 'low' && a.severity !== 'nominal')
  ).length;
  const criticalCount = anomaliesList.filter(
  ...
  const totalVessels =
    ...
    metricsData?.metrics?.maritimeCount ??
    (states as any)?.total ??
    188;
  const transitingCount =
    (states as any)?.underway ?? (states as any)?.transiting ?? Math.round(totalVessels * 0.65);
  const anchoredCount = (states as any)?.anchored ?? Math.round(totalVessels * 0.22);
  const maneuveringCount =
    (states as any)?.moored ?? (states as any)?.maneuvering ?? Math.round(totalVessels * 0.13);
    ...
    () => [
      {
        ...
        anomalyCount: anomaliesList.filter((a) => (a.region || '').toLowerCase().includes('hormuz'))
          .length,
      ...
      },
      {
        ...
        anomalyCount: anomaliesList.filter((a) =>
          (a.region || '').toLowerCase().includes('persian')
        ).length,
        riskLevel: 'medium',
        avgScore: 22,
      },
      ...
      },
      {
        ...
        anomalyCount: anomaliesList.filter((a) =>
          (a.region || '').toLowerCase().includes('fujairah')
        ).length,
        riskLevel: 'medium',
        avgScore: 31,
      },
  ...
  const selectedThreatItem: ThreatItem | null = useMemo(() => {
    ...
    return {
      ...
      description: Array.isArray(selectedAnomaly.reasons)
        ? selectedAnomaly.reasons.join('; ')
        : selectedAnomaly.reasons || 'Anomaly detected',
      ...
      reasons: Array.isArray(selectedAnomaly.reasons)
        ? selectedAnomaly.reasons
        : [selectedAnomaly.reasons || ''],
  ...
  const latestTimestamp =
    ...
    (tracesData?.traces?.[0] as any)?.updatedAt ||
    (tracesData?.traces?.[0] as any)?.timestamp;

  return (
    <div
      className={cn(
        'w-full max-w-[1700px] mx-auto px-2 sm:px-3 py-2 space-y-2.5 overflow-y-auto max-h-[calc(100vh-3.2rem)] select-none',
        className
      )}
    >
