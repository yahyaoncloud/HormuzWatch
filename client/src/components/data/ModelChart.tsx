  ...
  return [
    {
      ...
      label: xLabel || '',
      labelFont: `12px "Share Tech", sans-serif`,
      labelSize: xLabel ? 16 : 0,
    },
    {
      stroke: textColor,
      grid: { stroke: gridColor, width: 1 },
...
function ChartWrapper({
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <button
        onClick={onExpand}
        className="absolute top-2 right-2 z-10 p-1.5 rounded-none text-[var(--color-fg-muted)] hover:text-[var(--color-fg)] hover:bg-[var(--color-bg-elevated)] transition-colors"
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      for (const entry of entries) {
        if (entry.contentBoxSize && uplotRef.current) {
          const newW = Math.floor(entry.contentRect.width);
          if (newW > 0) {
            uplotRef.current.setSize({ width: newW, height: height });
          }
        }
      }
    });
...
export function FeatureImportanceChart({
  ...
  const buildOpts = useCallback(
    (w: number, h: number): uPlot.Options => {
      ...
      ax0.size = 24;
      ax1.font = `11px "Inter", system-ui, sans-serif`;
