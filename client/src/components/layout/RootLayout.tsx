    ...
    <div className="flex min-h-screen flex-col bg-[var(--color-bg)]">
      <Navbar>
        <div className="flex items-center gap-0 border border-[var(--color-border)] divide-x divide-[var(--color-border)] rounded-none">
          ...
          <button
            onClick={togglePanel}
            className="relative inline-flex items-center justify-center h-7.5 w-7.5 bg-[var(--color-bg-elevated)] text-[var(--color-fg)] hover:bg-[var(--color-bg-hover)] transition-all rounded-none"
            aria-label={`Notifications${unread > 0 ? ` (${unread} unread)` : ''}`}
          >
            <Bell className="h-3.5 w-3.5 text-[var(--color-fg-muted)]" />
            {unread > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[15px] h-3.5 bg-[var(--color-primary-600)] text-[9px] font-bold text-white flex items-center justify-center px-1 rounded-none">
                {unread > 99 ? '99+' : unread}
              </span>
            )}
          </button>
