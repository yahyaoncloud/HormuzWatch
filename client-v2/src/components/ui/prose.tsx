...
export function ProseBarChart({
  ...
  const w = data.length * 48 + 40;
  const h = height;
  const pad = { l: 36, r: 12, t: 10, b: 28 };

  return (
    <div className="prose-chart">
      {title && <div className="prose-chart-title">{title}</div>}
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
        {[0, 0.5, 1].map(p => {
          ...
          return (
            <g key={p}>
              ...
              <text x={pad.l - 4} y={y + 3} textAnchor="end" className="fill-[var(--color-fg-muted)]" fontSize="9" fontFamily="var(--font-data)">
                {val}
              </text>
            </g>
          );
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x} y={y} width={bw} height={barH} fill={d.color || 'var(--color-primary-600)'} opacity="0.85" />
              <text x={x + bw / 2} y={h - 8} textAnchor="middle" className="fill-[var(--color-fg-muted)]" fontSize="9" fontFamily="var(--font-ui)">
                {d.label}
              </text>
              <text x={x + bw / 2} y={y - 4} textAnchor="middle" className="fill-[var(--color-fg)]" fontSize="10" fontFamily="var(--font-data)" fontWeight="600">
                {d.value}
              </text>
            </g>
          );
      ...
      <div className="space-y-1.5">
        {data.map((d, i) => (
          <div key={i} className="flex items-center gap-2 text-[11px]">
            <span className="w-24 shrink-0 text-[var(--color-fg-muted)] text-right">{d.label}</span>
            <div className="flex-1 h-3 bg-[var(--color-bg-elevated)] overflow-hidden">
              <div
            ...
            <span className="w-8 shrink-0 font-mono text-[var(--color-fg)] text-right">{d.value}%</span>
          </div>
        ))}
      </div>
    </div>
